...
def prepare_data_for_regression(input_data,tourism_data):
    ...
    if 'PULocationID' not in taxi_grouped.columns:
    # rename 'PU Location ID' to 'PULocationID'
        taxi_grouped = taxi_grouped.rename(columns={'PUlocationID': 'PULocationID'})


    taxi_data = pd.merge(taxi_grouped, tourism_weekdays_climate, on=['date_pickup','PULocationID'], how='left')
    # drop duplicates ( for some locations there are double entries)
    taxi_data.drop_duplicates(subset = ["PULocationID", "date_pickup"],keep='first', inplace=True, ignore_index=True)
    ...
    # restrict taxi_data to temperature range -7 to 35 ------ HAVE TO MAKE SURE THIS RESTRICTION MAKES SENSE
    taxi_data_cut = taxi_data[(taxi_data['tmax_obs'] >= -7) & (taxi_data['tmax_obs'] <= 35)]
    
    taxi_data_cut.to_csv(f'{input_data[:input_data.find("/")]}/data_regression_PU.csv', index=False)
...
taxi_data = "High_Volume_FHV/merged_grouped_PU.csv"
